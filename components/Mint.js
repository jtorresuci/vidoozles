...
const Mint = () => {
  ...
  const [vidoozleList, setVidoozleList] = useState([]);
  const [loadedVidoozleList, setLoadedVidoozleList] = useState(false);
  const [formattedVidoozleList, setFormattedVidoozleList] = useState([]);

  // const handleChange = (e) => {
  //   var current = document.getElementById(e.value);
  //   var prev = document.getElementById(prevItem == undefined ? "" : prevItem.value);
  //   prev?.classList.remove('img-highlight');
  //   current?.classList.add('img-highlight');
  //   prevItem !== undefined && pumpkins.push(prevItem);
  //   let index = pumpkins.findIndex(p => p.value === e.value);
  //   setPrevItem(pumpkins[index]);
  ...
  async function connectWallet() {
    ...
    // console.log(walletOfSigner.length);
    // console.log(walletOfSigner.length);

    if (walletOfSigner.length > 0) sethasVidoozles(true);
    else sethasVidoozles(false);
    // console.log(hasVidoozles);

    // if(hasVidoozles) getVidoozles(c)
  }
  ...
  async function handleMint() {
    ...
    try {
      ...
      try {
        ...
        // console.log(options)
        // console.log(amount)
        let tx = await mintWithSigner.functions.mint(
          signerAddress,
          quantity,
          totalPrice
        );
        ...
        if (e.code == -32603) {
          let balanceOfSignerReadable = ethers.utils.formatEther(
            await signerObj.getBalance()
          );
  ...
  async function getVidoozles() {
    console.log(loadedVidoozleList)
    const c = new ethers.Contract(contractAddress, abi, signerObj);

    let walletOfSigner = await c.walletOfOwner(signerAddress)
    const listOfVidoozleNumbers = walletOfSigner.map((n) => n.toString())
    const formattedList = listOfVidoozleNumbers.map((n) => `Vidoozle #${n}`)

    setVidoozleList(listOfVidoozleNumbers);
    setFormattedVidoozleList(formattedList);

    console.log(formattedVidoozleList)
    if(listOfVidoozleNumbers.length > 0) setLoadedVidoozleList(true)
    console.log(loadedVidoozleList)




    // contract?.methods.walletOfOwner(wallet_address).call().then((list: Array<string>) => {
    //   console.log(typeof (list));
    //   console.log(list);
    //   list.map((items, i) => {
    //     let myObj: PumpkinOptions = {} as PumpkinOptions;
    //     myObj.value = items;
    //     myObj.label = `Pupmkin #${items}`;
    //     pupmkins_inv.push(myObj);
    //     pumpkins.push(myObj);
    //   });
    //   setPupmkinList(list);
    // }).catch(e) => console.log(`walletOfOwner: ${err}`));
  }

  return (
    <Box pl={["0", sidePadding]} pr={["0", sidePadding]} pt="5rem">
      ...
      >
        ...
        <Flex flexDir={"column"} alignItems={"center"}>
          <Image src="../gif.gif" borderRadius="30px" />
          <Text fontWeight={"bold"} p="1rem" fontSize={"1.35rem"}>
            {" "}
            Total minted: {supplyText} / 10007
          </Text>
        </Flex>
        <Box>
          <Center>
            <VStack p="2rem">
              {
                ...
                hasMetamask ? (
                  ...
                  <Box>
                    ...
                    <Link isExternal="true" href="https://metamask.io/">
                      https://metamask.io/
                    </Link>{" "}
                  </Box>
                )
              ...
              >
                <Center fontFamily={"BodoAmat"}>Connect to Web3</Center>
              </Button>
              <Flex flexDir={"row"} p="1rem">
              ...
              </Flex>
              {
                ...
                correctNetwork ? (
                  <Button
                    ...
                    onClick={handleMint}
                  >
                    <Center fontFamily={"BodoAmat"}>Mint {sliderValue}</Center>
                  </Button>
                ) : (
                  <div></div>
                )
      ...
      {hasVidoozles ? (
        <Box>
          ...
          <Center pb="2rem">$EMBER</Center>
          <Center>
          <Button onClick={(e) => getVidoozles()}>Load Inventory</Button>
          {
            loadedVidoozleList ? (<Select
              options={formattedVidoozleList}
              // className="select"
              placeholder="Select a pumpkin..."
              onChange={handleChange}
              // value={pumpkins.find(obj => obj.value === selectedValue2)}
            />) : (<div></div>)
          }
          </Center>
        </Box>
