...
const MintPaused = () => {
  ...
  const handleConnect = (e) => {
    window.ethereum
      ? ethereum
          ...
          .then(() => {
            ethereum.request({ method: "net_version" }).then((reply) => {
              if (reply === "137") {
                ...
                c.methods
                  ...
                  .call()
                  .then((_supply) => {
                    setSupplyText(_supply);
                  })
                  .catch((err) => console.log(err));
              } else {
                alert("Please switch to Matic network");
              }
            });
          })
          .catch((err) => console.log(err))
  ...
  return (
    <Box pl={["0", sidePadding]} pr={["0", sidePadding]} pt="5rem">
      <Center pb="3rem" fontSize={"20pt"} fontFamily={"BodoAmat"}>
        <VStack>
          <Text align={"center"}>To purchase a Vidoozle Visit our official </Text>
          <Link href="https://opensea.io/collection/vidoozles" >Opensea Page</Link>
        </VStack>
      </Center>
      <Grid
...
//       const _supply = await c.totalSupply();
//       setSupplyText(Number(_supply))
...
//     const mintWithSigner = contract.connect(signer);
//     const options = { value: _price.toString() }

//     try {
//       let tx = await mintWithSigner.mint(address, amount, options);
